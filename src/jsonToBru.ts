...
`;
    ...
    if (enabled(body.formUrlEncoded).length) {
      bru += `\n${indentString(
        enabled(body.formUrlEncoded)
          .map((item: any) => `${item.name}: ${encodeURIComponent(item.value)}`)
          .join("\n")
      )}`;
    }
  ...
  if (reqvars && reqvars.length) {
    ...
    if (varsDisabled.length) {
      bru += `\n${indentString(
        varsDisabled.map((item: any) => `~${item.name}: ${item.value}`).join("\n")
      )}`;
    }
  ...
  if (resvars && resvars.length) {
    ...
    if (varsEnabled.length) {
      bru += `\n${indentString(
        varsEnabled.map((item) => `${item.name}: ${item.value}`).join("\n")
      )}`;
    }
    ...
    if (varsDisabled.length) {
      bru += `\n${indentString(
        varsDisabled.map((item) => `~${item.name}: ${item.value}`).join("\n")
      )}`;
    }
